    ...
    <form action="" id="update-status">
        ...
        <div class="form-group">
            ...
            <select type="text" name="status" class="form-select form-select-sm" required>
                ...
                <option value="2" <?php echo $_GET['status'] == 2 ? "selected" : '' ?>>Arrived</option>
                <option value="3" <?php echo $_GET['status'] == 3 ? "selected" : '' ?>>Done</option>
                <option value="4" <?php echo $_GET['status'] == 3 ? "selected" : '' ?>>No Show</option>
                <option value="5" <?php echo $_GET['status'] == 3 ? "selected" : '' ?>>Cancelled</option>
            </select>
        </div>
    </form>
</div>
<script>
    $(function(){
        $('#update-status').submit(function(e){
            ...
            $.ajax({
                url:'../Actions.php?a=update_reservation_status',
                    ...
                    if(resp.status == 'success'){
                        ...
                        setTimeout(() => {
                            ...
                            if(resp.return_status == 1)
                                ...
                                new_stat = '<span class="badge bg-warning"><small>Arrived</small></span>';
                            else if(resp.return_status == 3)
                                new_stat = '<span class="badge bg-success"><small>Done</small></span>';
                            else if(resp.return_status == 4)
                                new_stat = '<span class="badge bg-danger"><small>No Show</small></span>';
                            else if(resp.return_status == 5)
